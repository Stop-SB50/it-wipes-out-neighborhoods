...
// DEFINE ICONS
var bus_icon = L.icon.mapkey({icon:"bus",color:'#725139',background:'#c6dcff',size:20});
var train_icon = L.icon.mapkey({icon:"train",color:'#725139',background:'#ff9b9b',size:20});
var ferry_icon = L.icon.mapkey({icon:"ship",color:'#725139',background:'#6cd873',size:20});
...
L.control.social({default_text: "Looks like I'm affected by SB50, how about you?"}).addTo(map);
...
var makerGroupLayer = L.featureGroup().addTo(map);
searchBox.addListener('places_changed', function() {
	...
	document.getElementById("search-address-button").style.backgroundColor = '#a8c9ff';
...
function search_another_address(){
	...
	document.getElementById("search-address-button").style.backgroundColor = "#ddd";
...
function showResults(affected, marker) {
	...
	var html = '<div style="padding-right:20px;"><b>' + searchAddress + "</b> is " + affected + '</div><span id="close" onclick="hideResults();">x</span>'
...
function checkIfAffected(marker) {
	...
	if (leafletPip.pointInLayer(markerLngLat, buildings_to_85ft_layer, true).length > 0) {
		...
		affected = 'within a jobs rich or good school area and could be <span style="color:red;font-weight:600;">upzoned to 75ft</span>. Note that these areas are provisional and may change as more information is released. Share this result using the social links at bottom left!';
	} else {
...
var busStopsLayer = L.geoJson(null, {
	pointToLayer: function(feature, latlng) {
		return new L.marker(latlng, {icon: bus_icon});
	},
	onEachFeature: function(feature, layer) {
		if (feature.properties.stop_name != null) {
			layer.bindTooltip(feature.properties.stop_name, {closeButton: false, offset: L.point(0, 0), classname: 'leaflet-tooltip'});
		}
	}
}).addTo(map);
...
var trainStopsLayer = L.geoJson(null, {
	pointToLayer: function(feature, latlng) {
		return new L.marker(latlng, {icon: train_icon});
	},
	onEachFeature: function(feature, layer) {
		if (feature.properties.stop_name != null) {
			layer.bindTooltip(feature.properties.stop_name, {closeButton: false, offset: L.point(0, 0), classname: 'leaflet-tooltip'});
...
var ferryTerminalsLayer = L.geoJson(null, {
	pointToLayer: function(feature, latlng) {
		return new L.marker(latlng, {icon: ferry_icon});
	},
	onEachFeature: function(feature, layer) {
		if (feature.properties.stop_name != null) {
			layer.bindTooltip(feature.properties.stop_name, {closeButton: false, offset: L.point(0, 0), classname: 'leaflet-tooltip'});
...
function scaleOpacity(zoomLevel) {
	if (zoomLevel > 10) {
		buildings_to_75ft_jobs_schools_layer.setStyle({'fillOpacity':0.5})
		buildings_to_75ft_rail_ferries_layer.setStyle({'fillOpacity':0.5})
		buildings_to_85ft_layer.setStyle({'fillOpacity':0.5})
		return
	}
	...
	buildings_to_75ft_jobs_schools_layer.setStyle({'fillOpacity':newOpacity})
	buildings_to_75ft_rail_ferries_layer.setStyle({'fillOpacity':newOpacity})
	buildings_to_85ft_layer.setStyle({'fillOpacity':newOpacity})
